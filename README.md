...
* **Register records as new before their relationships, in the same order**: Apex has no identity hash for sObjects, so the Unit of Work finds a record's place among the new records by scanning from where it last looked. Registering relationships in record order, in reverse, or as a group of parents followed by a group of children costs a comparison or two per relationship; an arbitrary order still works but can cost a pass over the type's new records per relationship. A relationship registered before its record is registered as new is resolved at commit and checked for duplicates by scanning the (normally empty) list of such relationships.
